...
class Fetch<TResponse> extends StatefulWidget {
  ...
  Fetch(
      ...
      : super(key: key) {
    if (this.name != null) {
      if (Dispatcher.existingOrNull<FetchController>(
              [FETCH_CONTROLLERS_BASE_KEY, name]) ==
          null)
        this._setupController();
      else {
        this._controllerDispatcher = Dispatcher<FetchController>(
            [FETCH_CONTROLLERS_BASE_KEY, this.name]);
      }
    }
  }
  ...
  final Function(SocketException? response)? onSocketError;
...
class _FetchState<TResponse> extends State<Fetch<TResponse>> {
  ...
  void _onDispatcherValueExpired(StoreChange<FetchCache<TResponse?>> param) {
    ...
    if (widget.createCache != null) {
      ...
      var existingDispatcher = widget.fetchCacheDispatchers
        ...
        var newDispatcher = Dispatcher<FetchCache<TResponse?>>(fetchCache.key)
            .listenOnUpdating(this._onDispatcherValueUpdating)
            .listenOnExpired(this._onDispatcherValueExpired);
  ...
  Future<void> _runRequest() async {
    ...
    try {
      ...
      errorSocket = e;
      if (widget.onSocketError != null) {
        widget.onSocketError!(errorSocket);
      }
  ...
  void _listenController(FetchControllerAction action, dynamic params) {
    switch (action) {
  ...
  void initState() {
    if (widget._controllerDispatcher != null) {
      final newController =
          FetchController(widget.name!, listener: _listenController);
