...
abstract class Store {
  ...
  static registerEvent<TPresenterValue>(dynamic rawKey,
      StoreListener<TPresenterValue> callback, StoreEvent storeEvent) {
    final key = StoreKeys.fromRaw(rawKey);
    final eventKey = StoreKeys.getEventKey(key, storeEvent);
    final eventEmitter =
        EventEmitter.addListener<StoreChange<TPresenterValue?>>(
            eventKey, callback);
  ...
  static unregisterEvent<TPresenterValue>(
      StoreListener<TPresenterValue> storeCallback) {
  ...
  static _emitExpireds<TValue>(String key, dynamic oldValue) {
    ...
    final eventParams =
        StoreChange<TValue>(key, oldValue: oldValue, newValue: null);
